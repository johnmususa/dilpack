...
inputBox.onkeyup = (e)=>{
	...
	if (userData) {
		emptyArray = suggestions.filter((data)=>{
			//filtering array value and user char to lowercase and return only words or sentence
			//which starts with user entered words
...
function showSuggestions(list) {
    ...
    if (!list.length) {
        userValue = inputBox.value;
        listData = '<li><a href="products/sleeves.html">' + userValue + '</a></li>';
    }
    else{
        listData = list.map((data) => {
            return '<li><a href="products/kraft.html">' + data + '</a></li>';
        }).join('');
    }
    suggBox.innerHTML = listData;
}